function [ cost, grad ] = denoisingAutoencoderCostSGD( obj, x )
...
%   grad : struct including W1grad, W2grad, b1grad and b2grad gradients
%
% orhanf
%%
...
if obj.tiedWeights
    z3 = obj.W1' * a2 + repmat(obj.b2, 1, nSamples); % pre-activation of output 
else
    z3 = obj.W2 * a2 + repmat(obj.b2, 1, nSamples); % pre-activation of output 
end
...
J_reg = (obj.lambda/2) .* ( sum(obj.W1(:).^2) + sum(obj.W2(:).^2));
...
if nargout>1
    ...
    if obj.tiedWeights
        delta2 = bsxfun(@plus, obj.W1 * delta3, beta_term) .* dNonLinearity(a2, obj.hActFun);
        W1grad = (1/nSamples) * ((a2 * delta3') + (delta2 * x' )) + (obj.lambda .* obj.W1);
        W2grad = [];
    else
        delta2 = bsxfun(@plus, obj.W2' * delta3, beta_term) .* dNonLinearity(a2, obj.hActFun);
        W1grad = (1/nSamples) * delta2 * x' + ( obj.lambda .* obj.W1 );    
        W2grad = (1/nSamples) * delta3 * a2' + ( obj.lambda .* obj.W2 );
    end
...
function kldiv = KL(p,q) 
    q(q==0) = 1e-6;      % numerical stability, to prevent inf
    q(q==1) = 1-1e-6;    % numerical stability, to prevent inf
